...
package org.friendlytalk.core.presentation.layouts
{
	...
	/**
	 * Replaces standard TileLayou, adds allignement features.
	 */
	public class TileLayoutExt extends LayoutBase
	{
		//--------------------------------------------------------------------------
		//
		//  Constructor
		 ...
		 */
		public function TileLayoutExt():void
		{
			super();
		}
		 ...
		 * 
		 *  <p>Set to -1 to allow the TileLayoutExt to determine
		 ...
		 * 
		 *  <p>Set to -1 to remove explicit override and allow the TileLayoutExt to determine
		...
		private function calculateColumnAndRowCount(width:Number, height:Number, elementCount:int):void
		{
			...
			else if (!isNaN(width) && (orientation == TileOrientation.ROWS || isNaN(height)))
			{
				...
				if (_columnCount > elementCount && elementCount > 0)
					_columnCount = elementCount;
			}
		 ...
		 *  visibleEndIndex based on the TileLayoutExt geometry values, like _columnWidth
		...
		override public function getNavigationDestinationIndex(currentIndex:int, navigationUnit:uint, arrowKeysWrapFocus:Boolean):int
		{
			...
			if (inRows)
			{
				// Is the TileLayoutExt initialized with valid values?
		...
		override public function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			//			_columnCount = Math.min(_columnCount, layoutTarget.numElements);
			
			xPos = (unscaledWidth - (_columnCount * _columnWidth + horizontalGap * (_columnCount - 1))) / 2;
			...
			for (var index:int = visibleStartIndex; index <= visibleEndIndex; index++)
			{
				...
				trace(counter, counterLimit);
				
				// Move along the minor axis
				if (++counter >= counterLimit)
				{
					var leftItems:uint = visibleEndIndex - index;
					...
					if (orientation == TileOrientation.ROWS)
					{
						...
						if (leftItems >= _columnCount)
							xPos = (unscaledWidth - (_columnCount * _columnWidth + horizontalGap * (_columnCount - 1))) / 2;
						else
							xPos = (unscaledWidth - (leftItems * _columnWidth + horizontalGap * (leftItems - 1))) / 2;
